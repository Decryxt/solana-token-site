...
export default function RevokeDelegateMobile() {
  ...
  const [tokenAccounts, setTokenAccounts] = useState([]); // [{ tokenAccount, mint, uiAmount, programId, hasDelegate }]
      ...
      try {
        ...
        const parse = (resp, programId) =>
          resp.value.map((acc) => {
            ...
            const delegate = info?.delegate ?? null; // present in parsed if set
            return {
        ...
        // Optional: show delegate accounts first
        combined.sort((a, b) => Number(b.hasDelegate) - Number(a.hasDelegate));
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (acct.owner.toBase58() !== publicKey.toBase58()) {
        setStatus("Connected wallet does not own this token account.");
        return alert("Your wallet does NOT own this token account.");
      }

      if (!acct.delegate) {
        setStatus("No delegate is set on this token account.");
        return alert("No delegate is set on this token account.");
      }
      ...
      if (!treasuryStr) {
        throw new Error(
          "Treasury wallet not configured (VITE_ORIGINFI_TREASURY)."
        );
      }
      const treasuryPubkey = new PublicKey(treasuryStr);

      const feeLamports = Math.round(REVOKE_FEE_SOL * LAMPORTS_PER_SOL);
      ...
      alert("Error revoking delegate: " + msg);
        ...
        <div className={`${surfaceInner} p-5 text-white`}>
          <div className="flex flex-col items-center text-center gap-3">
            <div className="h-12 w-12 rounded-xl border border-[#1CEAB9]/30 bg-black flex items-center justify-center">
              <FaUserTimes className="text-[#1CEAB9] text-2xl" />
            </div>

            <div>
              <div className="text-lg font-bold">Revoke Delegate</div>
          ...
          <div className="mt-4 text-sm text-white/75 text-center">
            Removes a delegate’s spending permission from a specific token account (ATA).
            Ownership is not changed — only delegate approval is removed.
          </div>
        </div>
      </div>
