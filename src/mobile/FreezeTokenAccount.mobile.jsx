...
export default function FreezeTokenAccountMobile() {
  const { publicKey, sendTransaction, connected } = useWallet();
  const { connection } = useConnection();

  const FREEZE_FEE_SOL = 0.02;

  const [searchTerm, setSearchTerm] = useState("");
  const [tokenAccounts, setTokenAccounts] = useState([]); // [{ pubkey, mint, amount, programId }]
  ...
  const handleFreeze = async () => {
    if (!selectedTokenAccount) return alert("Please select a token account to freeze.");
    ...
    try {
      ...
      // ✅ getAccount/getMint need the correct program id (classic vs 2022)
      ...
      const feeLamports = Math.round(FREEZE_FEE_SOL * LAMPORTS_PER_SOL);
      ...
      alert("Error freezing token account: " + msg);
      ...
      <div className={surface}>
        <div className={`${surfaceInner} p-5 text-white`}>
          <div className="flex flex-col items-center text-center gap-3">
            ...
            <div>
              <div className="text-lg font-bold">Freeze Token Account</div>
          ...
          <div className="mt-4 text-sm text-white/75 text-center">
            Freezing a token account disables transfers for that account until it is thawed.
      ...
      <div className="mt-4 space-y-4">
        <div className={surface}>
          <div className={`${surfaceInner} p-4 text-white space-y-3`}>
            <div className="text-sm font-semibold">Select Token Account</div>
