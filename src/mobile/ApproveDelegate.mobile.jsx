...
export default function ApproveDelegateMobile() {
  const { publicKey, sendTransaction, connected } = useWallet();
  const { connection } = useConnection();

  const APPROVE_FEE_SOL = 0.02;

  const [searchTerm, setSearchTerm] = useState("");
  const [tokenAccounts, setTokenAccounts] = useState([]); // [{ tokenAccount, mint, uiAmount, decimals, programId }]
  const [selectedTokenAccount, setSelectedTokenAccount] = useState("");
  const [delegateAddress, setDelegateAddress] = useState("");
  const [amount, setAmount] = useState(""); // UI units
  ...
  const handleConfirm = async () => {
    ...
    if (!selectedTokenAccount) return alert("Please select a token account (ATA).");
    ...
    try {
      ...
      // ✅ getAccount needs correct program (classic vs Token-2022)
      ...
      const feeLamports = Math.round(APPROVE_FEE_SOL * LAMPORTS_PER_SOL);
      ...
      const tx = new Transaction().add(
        ...
        // 2) Approve Delegate instruction
        ix
      );
      ...
      alert("Error approving delegate: " + msg);
        ...
        <div className={`${surfaceInner} p-5 text-white`}>
          <div className="flex flex-col items-center text-center gap-3">
            <div className="h-12 w-12 rounded-xl border border-[#1CEAB9]/30 bg-black flex items-center justify-center">
              <FaUserCheck className="text-[#1CEAB9] text-2xl" />
            </div>

            <div>
              <div className="text-lg font-bold">Approve Delegate</div>
