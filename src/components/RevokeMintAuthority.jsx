...
export default function RevokeMintAuthority({ onBack }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Pre-check: Is mint authority already revoked?
      if (!mintInfo.mintAuthority) {
        alert("⚠️ Mint authority is already revoked.");
        setProcessing(false);
        return;
      }

      // Pre-check: Does the connected wallet control mint authority?
      ...
      console.error(err);
      alert("❌ Error revoking mint authority: " + err.message);
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="flex flex-col h-full text-white">
      ...
      <div className="flex items-center justify-between mb-6 relative">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex items-center space-x-3 pointer-events-none">
          <FaBan className="text-[#1CEAB9] text-3xl" />
          <h2 className="text-3xl font-bold text-center">Revoke Mint Authority</h2>
        </div>
      ...
      <div className="flex flex-grow overflow-hidden">
        {/* Left side: vertically centered details */}
        <div className="w-1/2 pr-6 px-4 flex flex-col justify-center text-gray-300">
          <h3 className="text-xl font-semibold mb-4">Details</h3>
          <p className="mb-4 leading-relaxed">
            Revoking mint authority disables future minting, permanently fixing the token supply. Only the current mint authority can perform this action.
          </p>
        ...
        {/* Right side: token search and selection */}
