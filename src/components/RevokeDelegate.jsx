...
export default function RevokeDelegate({ onBack }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!acct.delegate) {
        alert("⚠️ No delegate is set on this token account.");
        return;
      }
      ...
      console.error(err);
      alert("❌ Error revoking delegate: " + (err?.message || err));
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="flex flex-col h-full text-white">
      <div className="flex items-center justify-between mb-6 relative">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex items-center space-x-3 pointer-events-none">
          <FaUserTimes className="text-[#1CEAB9] text-3xl" />
          <h2 className="text-3xl font-bold text-center">Revoke Delegate</h2>
        </div>
